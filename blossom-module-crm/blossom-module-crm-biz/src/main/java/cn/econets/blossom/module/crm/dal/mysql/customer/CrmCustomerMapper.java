...
public interface CrmCustomerMapper extends BaseMapperX<CrmCustomerDO> {
    ...
    default PageResult<CrmCustomerDO> selectTodayCustomerPage(CrmTodayCustomerPageReqVO pageReqVO, Long userId) {
        ...
        // 拼接自身的查询条件
        // TODO 这里不仅仅要获得 today、tomorrow。而是 today 要获取今天的 00:00:00 这种；
        ...
        if (pageReqVO.getContactStatus().equals(CrmContactStatusEnum.NEEDED_TODAY.getType())) {
            ...
            query.between(CrmCustomerDO::getContactNextTime, today, tomorrow)
                    // TODO 是不是查询 CrmCustomerDO::contactLastTime < today？因为今天联系过，应该会更新该字段，减少链表查询；
            ...
            // 1. 【客户】的【下一次联系时间】 <= 【昨天】
            // 2. 无法找到【今天】创建的【跟进】记录
            //  TODO 是不是 contactNextTime 在当前时间之前，且 contactLastTime < contactNextTime？说白了，就是下次联系时间超过当前时间，且最后联系时间没去联系；
